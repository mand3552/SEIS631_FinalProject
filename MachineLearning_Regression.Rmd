...
Looking at the dataset, we see it is already broked in to *train* and *test*, each with an x and y and both the x dataframes have 13 columns. If you remember from the description of Supervised learning above, we train the algorithm on input data and the desired output. The *test* data is the desired output we will be running the regression on. So let's move on and assign their variables:
